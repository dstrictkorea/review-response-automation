...
const HARDCODED_RULES: HardcodedRule[] = [
  ...
  {
    // 환불: 보고 화법(제3자 과거 — "친구가 환불했다는 얘기") 제외, 요구형은 전부 매칭
    pattern: /환불(?!\s*(?:했다(?:는|대|더|던|길래)|받았다(?:는|대|더|던)|얘기|이야기))|환급|보상|배상|변상|돌려\s*줘|돌려\s*달라|돈\s*돌려|물어\s*내|물어\s*줘|물어\s*달라/u,
    keyword: '[KO] 환불/보상요구',
    riskLevel: 'high',
    lang: 'ko',
  },
  ...
  {
    pattern: /injur(?:ed|y)|hurt\s+myself|hurt\s+(?:my\s+)?(?:child|kid|son|daughter)|accident\s+(?:happened|occurred)|broke\s+(?:my\s+)?(?:arm|leg|bone|wrist|ankle)|bleeding\b(?!\s*[-\s]?edge\b|\s*heart\b)|\bhospital\b(?!\s*(?:nurse|worker|staff|doctor|physician|administrator|employee)\b)|ambulance|emergency\s+room/i,
