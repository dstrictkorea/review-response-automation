...
- **전수(KO 250건) 자동 플래깅** 후 슬롯 직접 교체. BEFORE→AFTER: 과장 표현 92→**0**, 생경한 단어 39→**0**,
  과존칭 7→**0**, 극단적 사과 18→**0**, **중복 사과(한 답변에 사과 2회+) →0**.
- **인사/긍정 클로징 담백화**: "진심으로 감사드립니다/깊이 감사드립니다/소중한 시간을 내어…/진심으로 환영하며" →
  "감사합니다/찾아주셔서 감사합니다/와주셔서 고맙습니다". "영광입니다·더없이·정말 기쁩니다" 과한 환호 → "기뻐요/
  기쁘네요/다행이에요" 등 담백·자연체. (미러 생일·데이트·감동·힐링·분위기, slotE, 재방문, 동반자 포함.)
- **'보답하겠습니다' 전면 제거**(생경·문어체) → "찾아뵙겠습니다/채워가겠습니다/더 나아지겠습니다" 등.
- **불만 이중 사과 구조 해소**: 인사(A)가 사과를 담당하고 **피벗(slotC)은 행동(개선/조치/연락)만** 서술하도록
  CROWD/LAYOUT/DISPLAY/KIOSK/STAFF/VALUE/재방문/CS격리 KO 라인 재작성 → "…죄송합니다" + "…사과드립니다" 중복 제거.
  인사 사과도 1개·중간 강도로 통일(진심으로/깊이 → 죄송합니다/정말 죄송합니다). 안전(보상·CCTV·법적·징계 금지,
  CS격리 사람 연락)·EMERGENCY 불변 유지.
- **'고객님' 코퍼릿 톤 완화** → "말씀하신/남겨주신 상황" 등.
- 검증: regression-guard ✅ (tsc 0 · waterfall ALL PASS · loop **0/813**) · 전수 재점검 dup 0.0% · 금칙어 0 · 누출 0.

---

**✅ 이전 EPIC: 말투 1차 고도화 — 과한 극존칭·극단적 사과 제거, 상황별 톤 (Wave 28)**
...
- **전수 문맥 점검(샘플 아닌 500건 전체) 추가 수정**:
  - **'감동' 미러 모순 제거**: "감동도 실망도 아닌 그 사이"(중립) 리뷰에 "감동을 받으셨다니" echo가 달리던
    모순 5건 → `extractContextMirror`에 '감동 부정 맥락'(감동도 아닌/없/까진 아니) 가드 추가(힐링처럼).
  - **단문 저노력 불만 경량화**: "Meh"/"별로"/"비싸요"/"Dreamy"(저별점 오타) 등 ≤25자 리뷰 8건이 장문
    사과를 받던 문제 → `slotShortComplaint`(사과 없는 가벼운 1문장: 피드백 감사 + 개선 의지). 직원불만·이탈은
    제외(제대로 사과 유지). loop 길이 게이트(TOO_SHORT/TOO_SHORT_COMPLAINT)도 ≤25자 리뷰는 면제.
  - 전수 자동검증: 금칙어 0 · 작품/discover 누출 0 · 무거운 표현 0 · 피벗-토픽 불일치 0 · 감동 미러 모순 0.
