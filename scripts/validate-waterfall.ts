...
// buildStaticReply가 {branch_name}→"ARTE MUSEUM NEW YORK", {artwork}→"STAR"로 치환하는지 확인.
// 미치환 토큰 `{...}` 잔존 금지.
// NOTE: 단순 칭찬은 묻지 않은 공간 안내(slotC_general/{highlight_room})를 더 이상 붙이지 않는다
//       (no-TMI 정책). 칭찬 경로의 토큰 치환은 작품 언급 리뷰(slotC_artwork)로 검증하고,
//       {highlight_room} 바인딩은 S5(룸 특정 불만)에서 검증한다.
{
  // 작품 언급 칭찬 → isArtworkFocused → slotC_artwork({artwork}→대표작) 포함.
  //   (감각어 빛/물/소리를 피해 작품 라인이 감각 조각에 밀리지 않도록 한 텍스트로 토큰 치환만 검증)
  ...
  check('S4 AMNY {artwork} token substituted in EN reply', /STAR|WATERFALL|ETERNAL/i.test(replyEN), replyEN.slice(0, 120))
