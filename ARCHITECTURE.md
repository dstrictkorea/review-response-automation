...
LLM은 기본 엔진이 아니다. **결정론적 5-레이어 분류 + 5-슬롯 정적 조립**이 기본이고,
...
reviewProcessor (3-Tier 스마트 라우팅)
  ...
  SAFE / COMPLIMENT    → route='static'  5-슬롯 정적 답변 + ai_done (승인 불필요)
...
## 3. 9개 언어 네이티브 답변 엔진 (Rounds 36–41)
...
### 3.2 5-슬롯 조립 — `staticTemplates.ts` + `replyTemplates.ts`
```
SAFE/COMPLIMENT : [A 인사] [B 감정응답] [C? 작품/일반] [D? 피크힌트] [E 클로징]
COMPLAINT/EMERG : [A 사과] [B 수용확인] [C? 태그피벗] [D? 피크힌트] [E 미니클로징]
변형 선택: reviewId → 슬롯별 소수 해시(2,3,5,7,11) → KO 8변형 × 타언어 4변형 순환
SHORT 모드: 단문(≤40자) 긍정 → A+B+E 3슬롯 (TMI 방지)
```
- 전 슬롯 함수 9개 언어 변형 보유. `SLOT_C_PIVOTS` 13종 불만 피벗도 9개 언어 완비.
...
655건 합성 리뷰 (30개 언어 × 다인종/연령/시나리오) → `processReview()` 전수 통과 → 14종 검출기:
...
**현재 기준선: 0/655 이슈.** 엔진/템플릿 수정 시 이 루프가 회귀 게이트다:
```bash
npx tsx scripts/deep-learning-loop.ts 2>&1 | grep "이슈 있는 리뷰:"   # 0/655 필수
```
...
src/lib/
  ...
  staticTemplates.ts      슬롯 변형 풀 (9개 언어 × A/B/C/D/E + SLOT_C_PIVOTS 13종)
  replyTemplates.ts       buildStaticReply 5-슬롯 조립 + Kill-Switch
...
scripts/
  deep-learning-loop.ts   655건 합성 리뷰 × 14종 검출기 회귀 게이트
